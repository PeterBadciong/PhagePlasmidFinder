...
parser.add_argument('-c', '--phage_threshold', type=float, default=0.85, help='Plasmid and virus combined threshold (default: 0.85)')
...
# Run the first script (PPF1DX.py) with arguments from the wrapper script
script1_args = [
    'python3', 'PPF1DX.py',
    args.fasta_file,
    args.genomad_db,
...
log_to_console_and_file("Running (PPF1DX.py)...")
...
if os.path.exists(genomad_output):
    # Run the second script (PPF2DX.py) with only the required arguments
    script2_args = [
        'python3', 'PPF2DX.py',
        args.fasta_file,
        genomad_output,
	...
	'--phage_threshold', str(args.phage_threshold),
	'--gene_min', str(args.gene_min),
	'--percent_min', str(args.percent_min)
    ]

    log_to_console_and_file("Running (PPF2DX.py)...")
