...
def extract_scaffolds(genomad_output, output_folder, fasta_file, plasmid_threshold=0.80, phage_threshold=0.85, verbose=False):
    try:
        ...
        with open(combined_output_file, 'w') as combined_out:
            with open(classification_file, 'r') as tsvfile:
                ...
                for row in reader:
                    ...
                    # Extract if the sum of plasmid and virus score > phage_threshold
                    if virus_score > phage_threshold and plasmid_score > plasmid_threshold:
                        scaffold_data = extract_scaffold(fasta_file, row['seq_name'])
                        if scaffold_data:
                            combined_out.writelines(scaffold_data)
                            if verbose:
                                print(f"Scaffold {row['seq_name']} added based on combined plasmid and virus score threshold.")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--phage_threshold', type=float, default=0.85, help='Plasmid and virus combined threshold (default: 0.85)')
    ...
    combined_fasta = extract_scaffolds(args.genomad_output, args.output_dir, args.fasta_file, args.plasmid_threshold, args.phage_threshold, args.verbose)
